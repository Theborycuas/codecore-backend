...
import com.codecore.patient.testsupport.PatientAdminIntegrationTestConfiguration;
...
@SpringBootTest(
        classes = PatientAdminIntegrationTestConfiguration.class,
...
class PatientAdminControllerIT extends AbstractPatientHttpIntegrationTest {
    ...
    @Test
    void shouldCreateAndListPatientsWhenAdmin() {
        ...
        webTestClient.post()
                ...
                .bodyValue(new CreatePatientRequest(
                        ...
                        null,
                        null,
                        List.of(new ExternalIdentifierRequest("MRN", "P-100"))
                ))
                .exchange()
                .expectStatus().isCreated()
                .expectBody()
                .jsonPath("$.displayName").isEqualTo("María García")
                .jsonPath("$.status").isEqualTo("ACTIVE")
                .jsonPath("$.externalIdentifiers[0].type").isEqualTo("MRN")
    ...
    @Test
    void shouldArchiveAndActivatePatient() {
        TenantId tenantId = provisionTenant();
        String token = loginAs(tenantId, SystemRoleTemplate.ADMIN);
        String patientId = createPatient(token, "Archive Me", null);
        ...
        webTestClient.post()
                .uri(PatientAdminApiPaths.PATIENTS + "/" + patientId + "/activate")
                .header("Authorization", "Bearer " + token)
                .exchange()
                .expectStatus().isOk()
                .expectBody()
    ...
    void shouldReturn403WhenUserTriesToCreate() {
        ...
        webTestClient.post()
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .bodyValue(new CreatePatientRequest("No Create", null, null, null, null, List.of()))
                .exchange()
                .expectStatus().isForbidden();
    }

    @Test
    void shouldReturn404ForCrossTenantPatient() {
        ...
        String patientInB = createPatient(adminB, "Tenant B Patient", null);
    ...
    @Test
    void shouldUpdatePatientAndListArchived() {
        TenantId tenantId = provisionTenant();
        ...
        String patientId = createPatient(adminToken, "Original", null);

        webTestClient.put()
                .uri(PatientAdminApiPaths.PATIENTS + "/" + patientId)
                .header("Authorization", "Bearer " + adminToken)
                .contentType(MediaType.APPLICATION_JSON)
                .bodyValue(new UpdatePatientRequest("Renamed", null, null, null, null, List.of()))
                .exchange()
                .expectStatus().isOk()
                .expectBody()
        ...
        webTestClient.get()
                .uri(PatientAdminApiPaths.PATIENTS + "?status=ARCHIVED")
                .header("Authorization", "Bearer " + managerToken)
                .exchange()
                .expectStatus().isOk()
                .expectBody()
    ...
    private String createPatient(String token, String displayName, UUID primaryOrganizationId) {
        AtomicReference<String> idRef = new AtomicReference<>();
        webTestClient.post()
                .uri(PatientAdminApiPaths.PATIENTS)
                .header("Authorization", "Bearer " + token)
                .contentType(MediaType.APPLICATION_JSON)
                .bodyValue(new CreatePatientRequest(displayName, null, null, null, primaryOrganizationId, List.of()))
                .exchange()
                .expectStatus().isCreated()
                .expectBody()
