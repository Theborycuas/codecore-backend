...
import com.codecore.payment.testsupport.AbstractPaymentHttpIntegrationTest;
import com.codecore.payment.testsupport.PaymentAdministrationVerificationTestConfiguration;
...
/**
 * FASE 22.7 — end-to-end Payments verification (cierre slice Payment).
 */
@SpringBootTest(
        classes = PaymentAdministrationVerificationTestConfiguration.class,
...
class PaymentVerificationIT extends AbstractPaymentHttpIntegrationTest {
    ...
    @Test
    void verification1_fullPaymentLifecycleJourney() {
        TenantId tenantId = provisionTenant();
        String adminToken = loginAs(tenantId, SystemRoleTemplate.ADMIN);
        String invoiceId = createIssuedInvoice(adminToken, 15000);

        String paymentId = createPayment(adminToken, invoiceId, "USD", 15000, "CASH");

        webTestClient.get()
                .uri(PaymentAdminApiPaths.PAYMENTS + "?page=0&size=20")
                .header("Authorization", "Bearer " + adminToken)
                .exchange()
                .expectStatus().isOk()
                .expectBody()
                .jsonPath("$.content.length()").isEqualTo(1)
                .jsonPath("$.content[0].id").isEqualTo(paymentId)
                .jsonPath("$.content[0].status").isEqualTo("RECORDED");

        webTestClient.get()
                .uri(PaymentAdminApiPaths.PAYMENTS + "/" + paymentId)
                .header("Authorization", "Bearer " + adminToken)
                .exchange()
                .expectStatus().isOk()
                .expectBody()
                .jsonPath("$.invoiceId").isEqualTo(invoiceId)
                .jsonPath("$.amountMinor").isEqualTo(15000)
                .jsonPath("$.currency").isEqualTo("USD")
                .jsonPath("$.paymentMethodCode").isEqualTo("CASH")
                .jsonPath("$.status").isEqualTo("RECORDED");

        webTestClient.post()
                .uri(PaymentAdminApiPaths.PAYMENTS + "/" + paymentId + "/void")
                .header("Authorization", "Bearer " + adminToken)
                .exchange()
                .expectStatus().isOk()
                .expectBody()
        ...
        webTestClient.get()
                .uri(PaymentAdminApiPaths.PAYMENTS + "?status=VOIDED")
                .header("Authorization", "Bearer " + adminToken)
                .exchange()
                .expectStatus().isOk()
                .expectBody()
    ...
    @Test
    void verification2_readOnlyCannotCreatePayment() {
        TenantId tenantId = provisionTenant();
        String readerToken = loginAs(tenantId, SystemRoleTemplate.READ_ONLY);

        webTestClient.post()
                .uri(PaymentAdminApiPaths.PAYMENTS)
                .header("Authorization", "Bearer " + readerToken)
                .contentType(MediaType.APPLICATION_JSON)
                .bodyValue(new CreatePaymentRequest(UUID.randomUUID(), "USD", 100, null))
                .exchange()
    ...
    void verification3_crossTenantAccessReturns404() {
        ...
        String invoiceInB = createIssuedInvoice(adminB, 4200);
        String paymentInB = createPayment(adminB, invoiceInB, "USD", 4200, null);

        webTestClient.get()
                .uri(PaymentAdminApiPaths.PAYMENTS + "/" + paymentInB)
    ...
    @Test
    void verification4_invalidInvoiceReferenceReturns404() {
        TenantId tenantId = provisionTenant();
    ...
    @Test
    void verification5_managerCanVoidPerMatrix() {
        TenantId tenantId = provisionTenant();
        String adminToken = loginAs(tenantId, SystemRoleTemplate.ADMIN);
        String managerToken = loginAs(tenantId, SystemRoleTemplate.MANAGER);

        String invoiceId = createIssuedInvoice(adminToken, 9900);
        String paymentId = createPayment(managerToken, invoiceId, "USD", 9900, "WIRE_TRANSFER");
    ...
    @Test
    void verification6_openApiDocumentsPaymentsAdministrationSurface() {
        webTestClient.get()
                .uri("/v3/api-docs/" + PaymentOpenApiConfiguration.PAYMENTS_ADMINISTRATION_GROUP)
                .exchange()
                .expectStatus().isOk()
                .expectBody()
                .jsonPath("$.paths['" + PaymentAdminApiPaths.PAYMENTS + "']").exists();
    }

    @Test
    void verification7_unauthenticatedRequestsReturn401() {
        webTestClient.get()
                .uri(PaymentAdminApiPaths.PAYMENTS)
                .exchange()
        ...
        webTestClient.get()
                .uri(PaymentAdminApiPaths.PAYMENTS + "/" + UUID.randomUUID())
                .exchange()
                .expectStatus().isUnauthorized();
    }

    @Test
    void verification8_createAgainstDraftInvoiceReturns404() {
        TenantId tenantId = provisionTenant();
        String token = loginAs(tenantId, SystemRoleTemplate.ADMIN);
        String orgId = createOrganization(token, "DRAFT_ORG", "Draft Org");
        String patientId = createPatient(token, "Draft Patient");

        String draftInvoiceId = createInvoice(
                token,
                UUID.fromString(orgId),
                UUID.fromString(patientId),
                List.of(new InvoiceLineRequest("Unissued line", 500, null, null))
        );
        ...
        webTestClient.post()
                ...
                .header("Authorization", "Bearer " + token)
                .contentType(MediaType.APPLICATION_JSON)
                .bodyValue(new CreatePaymentRequest(UUID.fromString(draftInvoiceId), "USD", 500, null))
                .exchange()
                .expectStatus().isNotFound();
    }

    private String createIssuedInvoice(String token, long amountMinor) {
        String orgId = createOrganization(token, "ORG_" + UUID.randomUUID().toString().substring(0, 8), "Payer Org");
        String patientId = createPatient(token, "Patient " + UUID.randomUUID().toString().substring(0, 8));
        String invoiceId = createInvoice(
                token,
                UUID.fromString(orgId),
                UUID.fromString(patientId),
                List.of(new InvoiceLineRequest("Service", amountMinor, null, null))
        );
        webTestClient.post()
    ...
    private String createPayment(
        ...
        webTestClient.post()
                ...
                .header("Authorization", "Bearer " + token)
                .contentType(MediaType.APPLICATION_JSON)
                .bodyValue(new CreatePaymentRequest(UUID.fromString(invoiceId), currency, amountMinor, paymentMethodCode))
                .exchange()
                .expectStatus().isCreated()
                .expectBody()
                .jsonPath("$.id").value(id -> idRef.set(id.toString()));
