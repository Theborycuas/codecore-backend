...
import com.codecore.appointment.testsupport.AppointmentAdminIntegrationTestConfiguration;
...
@SpringBootTest(
        classes = AppointmentAdminIntegrationTestConfiguration.class,
...
class AppointmentAdminControllerIT extends AbstractAppointmentHttpIntegrationTest {
    ...
    void shouldCreateListUpdateCancelAndComplete() {
        ...
        AtomicReference<String> appointmentIdRef = new AtomicReference<>();
        webTestClient.post()
                ...
                .jsonPath("$.id").value(id -> appointmentIdRef.set(id.toString()));

        assertThat(appointmentIdRef.get()).isNotNull();
        ...
        webTestClient.put()
                .uri(AppointmentAdminApiPaths.APPOINTMENTS + "/" + appointmentIdRef.get())
                .header("Authorization", "Bearer " + token)
                .contentType(MediaType.APPLICATION_JSON)
        ...
        String completeId = createAppointment(
                token,
                fixture,
        ...
        webTestClient.post()
                .uri(AppointmentAdminApiPaths.APPOINTMENTS + "/" + completeId + "/complete")
                .header("Authorization", "Bearer " + token)
                .exchange()
                .expectStatus().isOk()
                .expectBody()
        ...
        webTestClient.post()
                .uri(AppointmentAdminApiPaths.APPOINTMENTS + "/" + appointmentIdRef.get() + "/cancel")
                .header("Authorization", "Bearer " + token)
                .exchange()
                .expectStatus().isOk()
                .expectBody()
        ...
        webTestClient.get()
                .uri(AppointmentAdminApiPaths.APPOINTMENTS + "?status=CANCELLED")
                .header("Authorization", "Bearer " + token)
                .exchange()
                .expectStatus().isOk()
                .expectBody()
    ...
    @Test
    void shouldReturn404ForCrossTenantAppointment() {
