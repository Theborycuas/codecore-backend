...
@Repository
public class R2dbcOrganizationAdminQueryRepository implements OrganizationAdminQueryRepository {

    private final DatabaseClient databaseClient;

    public R2dbcOrganizationAdminQueryRepository(ConnectionFactory connectionFactory) {
        this.databaseClient = DatabaseClient.create(connectionFactory);
    }

    @Override
    public Flux<AdminOrganizationView> findByTenantId(
            TenantId tenantId,
            StructureListFilter filter,
            PageQuery pageQuery
    ) {
        String orderColumn = PageQueryParser.organizationSqlOrderColumn(pageQuery.sortField());
        ...
        return databaseClient.sql("""
                        SELECT organization_id, tenant_id, code, name, status, created_at, updated_at
    ...
    @Override
    public Mono<Long> countByTenantId(TenantId tenantId, StructureListFilter filter) {
        String statusClause = statusClause(filter);
        return databaseClient.sql("""
                        SELECT COUNT(*)
                        FROM org.organization
